...
- **Отправка данных в сервис** — POST-запрос на `{base_url}/company/add/{code}/{title}` с заголовком `X-API-Key`. Если настроенный публичный адрес возвращает `404`, `5xx` или недоступен по сети, расширение повторяет сохранение через локальный API `http://127.0.0.1:8080`; для ответов `400`/`401` автоматический fallback не выполняется.
...
- **Автоматический запуск** — запрос отправляется после полной загрузки страницы. Для сохранения компании сначала используется URL из настроек, а при сетевой ошибке или ответе `404`/`5xx` выполняется повтор через `LOCAL_API_BASE_URL`; поэтому при работе на том же компьютере достаточно запустить REST API локально, даже если ngrok временно недоступен. Если оба адреса недоступны, нужно проверить API Service URL, локальный REST API и состояние ngrok. Бейджи не выводятся для статей, постов и новостей, которых ещё нет в базе данных сервиса (API возвращает `404 Not Found`).
...
| `CompanyApiClient` | `content_scripts/companyApiClient.js` | Работа с REST API: `sendCompany(code, title)` — POST-запрос добавления компании через настроенный URL с fallback на `LOCAL_API_BASE_URL` при сетевой ошибке или ответе `404`/`5xx`; `getStatuses(code)` — GET-запрос статусов компании; `getArticleStatuses(companyCode, articleId)` — GET-запрос статусов статьи; `getNewsStatuses(companyCode, newsId)` — GET-запрос статусов новости; `getPostsStatuses(companyCode, postIds)` — пакетный GET-запрос статусов постов (`?ids=...`); `updateCompanyStatus(code, field, direction)` / `updateArticleStatus(code, id, field, direction)` / `updateNewsStatus(code, id, field, direction)` / `updatePostStatus(code, id, field, direction)` — PATCH-запросы смены статуса на шаг вперёд (`fwd`) или назад (`back`); `addComment(data)` / `deleteComment(commentId)` — добавление/удаление закладки комментария. Для `addComment` на статье `data` также содержит `company_code` и `article_title`, чтобы REST API зарегистрировал статью перед сохранением; успешный ответ используется для подробного уведомления с ID комментария и данными созданной статьи. Base URL читается из настроек (`BrowserStorage`). |
...
export const CONFIG = {
    ...
    DEFAULT_BASE_URL: 'https://...',             // Публичный URL сервиса по умолчанию
    LOCAL_API_BASE_URL: 'http://127.0.0.1:8080', // Локальный fallback для сохранения компании
    URL_PATTERN: 'https://habr.com/ru/companies/*',  // Паттерн для срабатывания
};
```

> **Примечание:** `API_KEY`, `DEFAULT_BASE_URL` и `LOCAL_API_BASE_URL` задаются в коде расширения. `base_url` можно менять через страницу настроек расширения (хранится в `storage.local`). При сохранении компании расширение сначала обращается к сохранённому `base_url`, а затем при сетевой ошибке или ответе `404`/`5xx` пробует `LOCAL_API_BASE_URL`.
