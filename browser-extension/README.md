...
- **Отправка данных в сервис** — POST-запрос на `{base_url}/company/add/{code}/{title}` с заголовком `X-API-Key`. Все запросы выполняются только через настроенный публичный адрес сервиса, предоставленный ngrok; при `4xx`/`5xx`, сетевой ошибке или недоступности туннеля расширение показывает ошибку и не обращается к локальному адресу. Получение статусов компании, статьи, новости и постов также выполняется только через этот публичный адрес.
- **Отображение ответа** — ответ сервиса выводится во всплывающем сообщении на странице через `MessageControl`
- **Индикаторы статусов компании** — после отправки данных запрашиваются статусы (`GET {base_url}/company/statuses/{code}`) и рядом с названием компании выводятся компактные стеклянные бейджи со значениями `action_dev`, `action_industry` и `action_company` (человекочитаемые `title` из справочника `statuses`; подписи — «Разработка», «Отрасль», «Компания»). Бейджи используют тёмную палитру UI Kit UI kit, цветные семантические точки и круглые кнопки навигации «◀»/«▶» с hover/focus-состояниями.
- **Совместимость с бесплатным ngrok** — все запросы к REST API проходят через фоновый обработчик и автоматически получают заголовок `ngrok-skip-browser-warning: true`. Он отключает HTML interstitial-страницу бесплатного ngrok, чтобы вместо неё расширение получало JSON-ответ API. Если endpoint ngrok offline или возвращает HTML вместо JSON, расширение не переключается на локальный API: необходимо восстановить туннель или изменить публичный `API Service URL` в настройках.
...
- **Синхронизация закладок комментариев** — при клике на значок закладки комментария (★) на страницах постов, статей или новостей компаний расширение автоматически добавляет или удаляет комментарий в закладки через REST API (POST /comment/add или DELETE /comment/{id}). Полный текст комментария извлекается из DOM без ограничения в 500 символов, включая актуальный AJAX-контейнер Habr `.tm-comment__body-content` (с fallback на `.comment-body`, `.comment-content`, `.message` и `.tm-comment__body`). После клика расширение несколько раз повторяет поиск текста, чтобы дождаться завершения AJAX-загрузки; идентификатор сущности и её ID определяются из URL страницы или мета-тега `og:url`. Для статьи дополнительно передаются `company_code` из URL и `article_title` из `h1.tm-title` (fallback — первый `h1`); сервер использует их для создания отсутствующей записи статьи до сохранения комментария. Если заголовок отсутствует, пустой или не читается из DOM, расширение записывает диагностическую ошибку, показывает сообщение «Не удалось определить заголовок статьи на странице Habr» и не отправляет неполный запрос. Если после ожидания AJAX текст комментария так и не найден, расширение записывает диагностическую ошибку, показывает сообщение «Не удалось определить текст комментария на странице Habr» и также не отправляет запрос. Перед отправкой комментария расширение записывает в консоль тип и ID сущности, ID комментария, код компании и признак наличия `article_title` (сам текст комментария и секретный ключ не логируются). После успешного ответа верхний бар показывает ID сохранённого комментария; если REST API создал статью, к уведомлению добавляются её ID, title и код компании. Если REST API отклоняет запрос с `4xx/5xx`, пользователю показывается HTTP-статус и текст ошибки из JSON-ответа, а подробности ответа записываются в консоль.
- **Автоматический запуск** — запрос отправляется после полной загрузки страницы и всегда выполняется через публичный URL из настроек. При сетевой ошибке, ответе `4xx`/`5xx` или HTML-ответе от неработающего ngrok расширение прекращает обработку без локального повтора; нужно проверить `API Service URL` и состояние туннеля. Бейджи не выводятся для статей, постов и новостей, которых ещё нет в базе данных сервиса (API возвращает `404 Not Found`), либо если публичный endpoint ngrok недоступен.

- **Быстрое добавление компании/отрасли через выделение текста** — на страницах Habr компаний (`https://habr.com/ru/companies/*`) при выделении текста появляется плавающая стеклянная панель с кнопками «Компания» и «Отрасль» для сохранения выделенного текста (`POST /company/quick-add` или `POST /category/quick-add`). Сервер автоматически транслитерирует название в код (например, "Яндекс" → "Yandeks"). Панель использует палитру UI Kit UI kit: violet-акцент для компании, cyan-акцент для отрасли, отдельную круглую кнопку закрытия и цветную рамку для результата (зелёный — успех, красный — ошибка). На других страницах панель не инициализируется. Панель закрепляется относительно окна браузера и располагается на расстоянии 6 px от выделения, а если сверху недостаточно места — под выделением.

## Визуальная система

Визуальные компоненты расширения следуют правилам `uikit/`: тёмные slate-поверхности, полупрозрачный glassmorphism с `backdrop-filter`, тонкие полупрозрачные границы, скругления и мягкие тени. Основной акцент — violet (`#8b5cf6`/`#a78bfa`), вторичный акцент — cyan (`#22d3ee`); состояния успеха и ошибки обозначаются emerald и rose. Общие токены находятся в `content/extension.css`, а стили плавающей панели — в `content/popup.css`, чтобы content-script не зависел от inline-оформления.

Статус-бар `MessageControl` сохраняет прежнюю логику показа и таймер, но теперь отображается как центрированная glass-карточка с вариантами `success`, `error` и `info`. Бейджи сохраняют прежнюю структуру данных и PATCH-навигацию; изменены только поверхность, кнопки и семантические цвета. Текстовое поле `.habr-status-badge__text` имеет фиксированную ширину 92 px, а длинные значения обрезаются с многоточием, поэтому переключение статуса не меняет ширину бейджа. Панель выделения сохраняет позиционирование и API-вызовы, но использует классы `habr-selection-panel*` вместо inline-стилей.
...
| `CompanyApiClient` | `content_scripts/companyApiClient.js` | Работа с REST API через единственный публичный `base_url`: `sendCompany(code, title)` — POST-запрос добавления компании; `getStatuses(code)`, `getArticleStatuses(companyCode, articleId)`, `getNewsStatuses(companyCode, newsId)` и `getPostsStatuses(companyCode, postIds)` — GET-запросы статусов; `updateCompanyStatus(code, field, direction)` / `updateArticleStatus(code, id, field, direction)` / `updateNewsStatus(code, id, field, direction)` / `updatePostStatus(code, id, field, direction)` — PATCH-запросы смены статуса на шаг вперёд (`fwd`) или назад (`back`); `addComment(data)` / `deleteComment(commentId)` — добавление/удаление закладки комментария. При сетевых и HTTP-ошибках fallback на локальный адрес не выполняется. Для `addComment` на статье `data` также содержит `company_code` и `article_title`, чтобы REST API зарегистрировал статью перед сохранением; успешный ответ используется для подробного уведомления с ID комментария и данными созданной статьи. Base URL читается из настроек (`BrowserStorage`). |
...
export const CONFIG = {
    ...
    DEFAULT_BASE_URL: 'https://...',             // Публичный URL ngrok по умолчанию
    URL_PATTERN: 'https://habr.com/ru/companies/*',  // Паттерн для срабатывания
};
```

> **Примечание:** `API_KEY` и `DEFAULT_BASE_URL` задаются в коде расширения. `base_url` можно менять через страницу настроек расширения (хранится в `storage.local`). Все запросы, включая сохранение компании, получение и изменение статусов, операции с комментариями и быстрые действия, отправляются только на сохранённый публичный `base_url`; локальный fallback отсутствует.
...
Заголовок `ngrok-skip-browser-warning` добавляется ко всем запросам `CompanyApiClient`, включая GET статусов, PATCH смены статусов и операции с закладками комментариев, а также к запросам панели быстрого добавления по выделению текста. Расширение использует только настроенный публичный `base_url`: при `404`/`5xx`, транспортной ошибке или HTML вместо JSON запрос завершается без повтора на другой адрес; ответы `400`/`401` также возвращаются как ошибки запроса или авторизации.
...
Значения `title` выводятся в бейджах рядом с названием компании на странице профиля. Порядок бейджей: `action_dev` (Разработка), `action_industry` (Отрасль), `action_company` (Компания).
...
- `field` — имя action-поля: `action_dev`, `action_post`, `action_comment`,
  `action_industry`, `action_company`. Для компании допустимы `action_dev`,
