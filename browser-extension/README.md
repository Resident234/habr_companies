...
Статус-бар `MessageControl` сохраняет прежнюю логику показа и таймер, но теперь отображается как центрированная glass-карточка с вариантами `success`, `error` и `info`. Бейджи сохраняют прежнюю структуру данных и PATCH-навигацию; изменены только поверхность, кнопки и семантические цвета. Панель выделения сохраняет позиционирование и API-вызовы, но использует классы `habr-selection-panel*` вместо inline-стилей.
