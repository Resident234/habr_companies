...
8. Получает публичный HTTPS-URL через ngrok API (`localhost:4040`)
...
2. Останавливает оставшийся процесс ngrok, если он ещё работает;
3. Автоматически перезапускает полный стек через `scripts/start_all.ps1`.
...
неисправном ngrok или недоступной сети.
