...
**Примечание:** сервер генерирует `code` из `title` с помощью транслитерации кириллицы в латиницу. Для `POST /category/quick-add` перед генерацией кода и сохранением сервер приводит выделенное название отрасли к именительному падежу с помощью `github.com/jus1d/gomorphy` и согласует определения с существительным. Например, `Информационной безопасности` сохраняется как `Информационная безопасность`, а код строится уже из нормализованного названия. `browser-extension` передаёт в этот эндпоинт исходный выделенный текст без склонения и другой лингвистической обработки. Если слово отсутствует в словаре или морфологический анализатор недоступен, исходное название сохраняется без преобразования.
...
- Требуется ngrok Agent версии `3.20.0` или новее; скрипт использует найденный `ngrok.exe` из `NGROK_EXE`, `PATH`, стандартных каталогов или установки winget
...
4. Проверяет дату `rest-api.exe` относительно всех исходников Go и файлов модулей, а также сравнивает встроенную в бинарник Git-ревизию с текущим `HEAD`; при отсутствии, устаревании или несовпадении ревизий выполняет `go build ./operate`, затем запускает актуальный `rest-api.exe`.
5. Создаёт отдельный совместимый конфигурационный файл ngrok в `%TEMP%\habr_companies_ngrok.yml`, чтобы старый пользовательский `ngrok.yml` не ломал запуск.
6. Ждёт готовности сервера (TCP-проверка порта 8080).
7. Запускает `ngrok http 8080`.
8. Получает публичный HTTPS-URL через ngrok API (`localhost:4040`). Если ngrok завершается до создания туннеля или URL не найден, скрипт останавливает запущенный им rest-api и завершается с ошибкой; причина берётся из `%TEMP%\ngrok_startall.err.log`.
9. Делает тестовый POST-запрос и выводит результат.
10. Печатает сводку: локальный адрес, публичный URL и пример curl; API-ключ в консоль не выводится.

**Выход:** Ctrl+C в окне скрипта останавливает оба процесса.

> **Важно:** скрипт больше не использует найденный `rest-api.exe` вслепую. Перед запуском он проверяет не только даты исходников и файлов модулей, но и встроенную Git-ревизию, которую Go записывает в бинарник. Поэтому checkout с теми же или более ранними датами файлов всё равно приводит к пересборке, если бинарник собран из другого коммита или без метаданных VCS. Это предотвращает `404 Not Found` на маршрутах, которые уже есть в исходном коде, но отсутствуют в старом исполняемом файле. Если Git недоступен или каталог не является checkout-ом, сохраняется проверка по датам. Скрипт также использует отдельный конфигурационный файл ngrok и не зависит от несовместимого старого пользовательского `ngrok.yml`. После обновления кода достаточно снова запустить `start_all.ps1`; вручную удалять `rest-api.exe` не требуется.
...
Для морфологического анализа используется `github.com/jus1d/gomorphy` версии `v0.2.3`. Словарь встраивается в исполняемый файл, поэтому REST API не требует отдельного Python runtime или внешнего сервиса. В результате размер бинарника увеличивается примерно на 8,8 МБ. Исходный код пакета распространяется под MIT, встроенные данные OpenCorpora — под CC BY-SA 4.0; при распространении бинарника необходимо сохранить соответствующие уведомления о лицензии.

Анализатор инициализируется лениво и безопасен для конкурентного использования после инициализации. Для фраз сохраняется число исходного существительного, а именительная форма выбирается по грамматическим тегам `nomn` и исходному числу, а не по фиксированной позиции в списке форм. Например, `Информационной безопасности` преобразуется в `Информационная безопасность`, `Информационных технологий` — в `Информационные технологии`, а `научно-исследовательских работ` — в `научно-исследовательские работы`. Дефисные составные существительные также лемматизируются через `WordForms`: `интернет-технологий` преобразуется в `интернет-технологии`, а `онлайн-сервисов` — в `онлайн-сервисы`. Для неизвестных слов применяется безопасный fallback: исходная строка передаётся дальше без изменений.

Для отладки сложных случаев можно включить подробное логирование переменной окружения `CATEGORY_NORMALIZATION_DEBUG`. Значения `1`, `true`, `yes` и `on` включают записи с префиксом `[category-normalization]`; по умолчанию логирование выключено. В записи указываются исходная фраза, найденное головное слово, его индекс, число, количество сгенерированных форм, выбранная форма и итоговый результат. Для дефисного составного существительного дополнительно указываются `compound=true` и исходный морфологический тег. При fallback дополнительно указывается причина (`empty`, `analyzer_unavailable`, `head_not_found` или `form_not_found`). Поскольку в лог попадает пользовательское название отрасли, включайте этот режим только на время диагностики.

Пример:

```powershell
$env:CATEGORY_NORMALIZATION_DEBUG = "1"
.\rest-api.exe
```

```text
[category-normalization] input="Информационной безопасности" head="безопасности" head_index=1 number=sing forms=9 selected_form=1 result="Информационная безопасность"
```
...
> Скрипт сам находит `.env`, пересобирает устаревший или собранный из другой Git-ревизии `rest-api.exe`, при необходимости находит или скачивает ngrok
> и использует отдельный совместимый конфигурационный файл. Для публичного туннеля требуется ngrok Agent `3.20.0+`.
> При старте он останавливает предыдущие процессы rest-api/ngrok, поэтому конфликтов портов не бывает.
...
| `scripts/watchdog.ps1` | Контролирует процессы и активный туннель, автоматически перезапускает стек |
| `scripts/start_run.log` | Вывод и код завершения последнего перезапуска, инициированного watchdog |
...
проверяет наличие процесса `rest-api.exe`, процесса `ngrok.exe` и хотя бы одного
активного туннеля через локальный management API ngrok
(`http://127.0.0.1:4040/api/tunnels`). Поэтому живой процесс ngrok без активного
туннеля считается неисправным — в том числе после `ERR_NGROK_9040` или потери
сессии. При пропаже любого процесса или туннеля watchdog:

1. Записывает в `scripts/watchdog.log` точное время и конкретную причину (`rest-api`, `ngrok process` или `ngrok active tunnel`).
2. Останавливает оставшийся процесс ngrok, даже если сам процесс ещё жив, но туннель уже недоступен.
3. Ждёт освобождения mutex предыдущего `start_all.ps1`, чтобы перезапуск не завершился ошибкой «Another start_all.ps1 instance is already running».
4. Автоматически перезапускает полный стек через `scripts/start_all.ps1`, сохраняя его вывод и код завершения в `scripts/start_run.log`.

Между попытками действует 30-секундная пауза, а пока предыдущий запуск ещё выполняется,
новая задача не создаётся. Это предотвращает бесконечное размножение процессов при
неисправном ngrok или недоступной сети. Если ngrok отклоняет подключение на стороне
сервиса (например, `ERR_NGROK_9040`), watchdog будет повторять попытки, но устранить
ограничение учётной записи или IP-адреса самостоятельно не может.
...
поэтому watchdog можно не трогать — он просто зафиксирует перезапуск в логе. Если в
`%TEMP%\ngrok_startall.err.log` появляется `ERR_NGROK_121`, обновите ngrok Agent до
версии `3.20.0+`; watchdog не сможет исправить неподдерживаемую версию самостоятельно.
...
| `%TEMP%\ngrok_startall.log` | запросы к туннелю и диагностические сообщения ngrok |
| `%TEMP%\ngrok_startall.err.log` | ошибки запуска ngrok; `ERR_NGROK_121` означает, что Agent нужно обновить до версии `3.20.0+` |
