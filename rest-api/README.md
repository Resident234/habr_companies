...
> **Важно:** скрипт больше не использует найденный `rest-api.exe` вслепую. Если исходники или файлы модулей были изменены после сборки, бинарник пересобирается перед запуском. Это предотвращает `404 Not Found` на маршрутах, которые уже есть в исходном коде, но отсутствуют в старом исполняемом файле. Скрипт также использует отдельный конфигурационный файл ngrok и не зависит от несовместимого старого пользовательского `ngrok.yml`. После обновления кода достаточно снова запустить `start_all.ps1`; вручную удалять `rest-api.exe` не требуется.
...
> Скрипт сам находит `.env`, пересобирает устаревший `rest-api.exe`, при необходимости находит или скачивает ngrok
...
проверяет наличие процессов `rest-api.exe` и `ngrok.exe`. При пропаже любого процесса
watchdog:

1. Записывает в `scripts/watchdog.log` точное время и список пропавших процессов;
