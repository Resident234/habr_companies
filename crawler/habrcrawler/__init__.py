...
__title__ = 'habrcrawler'
__author__ = 'Resident234'
__license__ = 'Apache 2.0'
__copyright__ = 'Copyright 2016-2026 Resident234'
...
class Crawler:
    def __init__(self, load=None, no_test=False, paused=False):
        ...
        self.habr_banners_mode = bool(config.read('Habr', 'BannersMode'))
        ...
        if load is not None:
            ...
            elif self.habr_articles_mode:
                # Habr articles mode: crawl company articles by numeric ID range
                self._seeds = []
                self.habr_generator = self.loop.run_until_complete(
                    habr_seeds.seed_from_database(self))
            else:
    ...
    def get_savefilename(self):
        savefile = config.read('Save', 'Name') or 'habrcrawler-save-$$'
    ...
    async def crawl(self):
        ...
        while True:
            ...
            if self.habr_generator is not None:
                # lazily queue the next batch of article urls
                self.habr_generator.maybe_top_up()

            if self.scheduler.done(len(self.workers)):
                if self.habr_generator is not None and not self.habr_generator.exhausted:
                    # queue drained but we still have urls to generate
                    self.habr_generator.maybe_top_up()
                    continue
