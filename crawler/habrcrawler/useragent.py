...
UserAgent:
  Style: laptopplus
  MyPrefix: test
  URL: http://habrcrawler.com/habrcrawler.html

This produces a string of 'test-habrcrawler' for (1), and
the user agent for (2) will contain test-habrcrawler/VERSION
and the URL.
...
def useragent(version):
    ...
    basic = '{}-habrcrawler/{} (+{})'.format(uac['MyPrefix'], version, uac['URL'])

    style = uac['Style']

    # check a few things I'd like to encourage / discourage
    if 'habrcrawler.com' in basic:
    ...
    robotname = uac['MyPrefix'] + '-habrcrawler'
