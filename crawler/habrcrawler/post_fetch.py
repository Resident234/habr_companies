...
parse links and embeds, using an algorithm chosen in conf -- habrcrawler.habrcrawler

parent subsequently calls add_url on them -- habrcrawler.habrcrawler
'''
...
async def post_2xx(f, url, ridealong, priority, host_geoip, json_log, crawler):
    ...
    if content_type not in html_types:
        ...
        if config.habr_mode_enabled():
            ...
            elif ridealong.get('links_page'):
                # Company profile page: extract widget links
                try:
                    saved = await company_links.parse_and_save_links(
                        body, company_code)
                    json_log['habr_links_saved'] = bool(saved)
                except Exception as e:
            ...
            elif ridealong.get('article_id') is not None:
                # Article detail page: use the h1 title from the full article.
                try:
                    saved = await habr_parse.parse_and_save(
                        body, url.url, company_code,
                        article_hubs=ridealong.get('article_hubs'))
