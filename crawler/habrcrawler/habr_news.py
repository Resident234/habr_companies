'''
Seed generation for the Habr companies news parser.

URLs follow the template:
    https://habr.com/ru/companies/{company}/news/{news_id}/

Works exactly like habr_seeds.py (articles), but iterates news ids and
tracks per-company progress in companies.last_processed_news_id.
'''
import logging
...
NEWS_URL_TEMPLATE = 'https://habr.com/ru/companies/{company}/news/{news_id}/'


class HabrNewsSeedGenerator:
    '''
    Lazily generates news urls for every company in the database,
    round-robin across companies (same mechanics as HabrSeedGenerator).
    '''

    def __init__(self, crawler):
        self.crawler = crawler
        self.template = config.read('Habr', 'NewsUrlTemplate') or NEWS_URL_TEMPLATE
        self.id_start = int(config.read('Habr', 'NewsIdStart') or 1)
        self.id_end = int(config.read('Habr', 'NewsIdEnd') or 10000000)
        self.batch_size = int(config.read('Habr', 'SeedBatchSize') or 20000)
        self.companies = []
        # Round-robin state: track next_id for each company separately
        self.company_next_ids = {}  # {company_code: next_news_id}
    ...
    async def setup(self):
        self.companies = await db.get_companies_news_progress()
        ...
        # saved progress (last_processed_news_id + 1), falling back to
    ...
    def _queue_batch(self):
        ...
        one news id from each company in turn, so progress is uniform
        ...
        while queued < self.batch_size:
            ...
            news_id = self.company_next_ids[company]
            self.company_next_ids[company] = news_id + 1

            # Advance round-robin index for next call
            self.rr_index = (self.rr_index + 1) % len(self.company_order)

            url = self.template.format(company=company, news_id=news_id)
            ridealong = {
                'url': URL(url),
                'priority': 1,
                'seed': True,
        ...
        LOGGER.debug('queued batch of %d news urls (round-robin, %d companies active)',
...
async def seed_from_database(crawler):
    '''
    Replacement for seeds.expand_seeds_config() when Habr news mode is on.
    '''
