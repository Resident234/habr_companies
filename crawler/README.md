...
безопасен: статьи с уже существующим `id` пропускаются (однако для новостей при повторном запуске поля `title`, `stats_counter`, `score_counter`, `bookmarks_counter` и `comments_counter` обновляются для уже существующих записей).
...
### Обновление существующих записей в news

В режиме сбора новостей (`NewsPagesMode` или `NewsMode`) теперь поддерживается обновление существующих записей в таблице `news`. Если новость с таким `id` уже существует в базе, при парсинге обновляются следующие поля:
- `title`
- `stats_counter`
- `score_counter`
- `bookmarks_counter`
...
Связи с хабами (`news_hubs`) при этом не удаляются, а добавляются (через `ON DUPLICATE KEY UPDATE`).

При обновлении дубликата в лог выводится информационное сообщение вида: `updated existing news {news_id} for company {company_code}`.
...
получения id. Заголовок новости (`title`) теперь извлекается напрямую с детальной страницы из `h1.tm-title.tm-title_h1` внутри блока `div.tm-article-presenter__header` (так как в списке новостей он часто пуст или обёрнут в разные теги, а на детальной странице может присутствовать несколько заголовков). Пустой метод `_extract_title` из парсера списка новостей удалён за ненадобностью. Найденные новости сохраняются в таблицу `news` (поля
аналогичны `articles`, за исключением отсутствия label). В режиме `NewsPagesMode` заголовок новости также успешно извлекается со страницы списка из элемента `h2.tm-title.tm-title_h2` внутри превью-блока новости. Хабы новости
