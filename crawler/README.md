...
The crawler uses the existing `habrcrawler.stats` infrastructure and emits a consolidated report from `Crawler.minute()` once per minute. The report keeps cumulative counters, current gauges, and interval rates in the same log stream; it does not create a second monitoring subsystem.

| Metric | Interpretation |
|---|---|
| `fetch ClientError` | Cumulative count of retryable `aiohttp.ClientError` failures. |
...
| `pages per minute` | Successful HTTP responses counted by `fetch URLs` during the latest reporting interval. |
| `client errors per minute` | New `fetch ClientError` failures during the latest reporting interval. |
